...
import pandas as pd
import warnings
from belt_position.algorithm.belt_position.calculate_raw_metrics import calculate_unfiltered_belt_metrics
from belt_position.algorithm.signal_processing.merge_belt_and_chest_data import merge_belt_chest
...
from belt_position.services.data_loading.merge_baseline_frame_data import merge_frame_with_baseline
...
from belt_position.algorithm.signal_processing.interpolate_belt_position import interpolate_belt_positions
from belt_position.algorithm.visualization.animate_estimated_belt_position import create_belt_position_animations
...
def run_belt_position_workflow():
    ...
    frame_baseline_merged_df = merge_frame_with_baseline(cleaned_frame_df, data["baseline"])
    ...
    interpolated_belt_chest_data = merge_belt_chest(interpolated_belt_data, chest_data, cfg.DATA_PROCESSED / "interpolated_belt_chest_data.xlsx")
...
def parse_cli():
    ...
    parser.add_argument("--test-id", type=str, required=True,
                        help="Override TEST_ID from config")
    parser.add_argument("--data-path", type=str, required=False,
                        help="Override DATA_ROOT_DIR from config")
    parser.add_argument("--start-time", type=float, required=False,
                        help="Override START_TIME from config")
    parser.add_argument("--end-time", type=float, required=False,
                        help="Override END_TIME from config")
    parser.add_argument("--pressure-threshold", type=float, required=False,
                        help="Override pressure threshold from config")
    parser.add_argument("--pressure-unit", required=False,
                        help="Override pressure unit from config")
    parser.add_argument("--minimum-points", type=int, required=False,
                        help="Override minimum points from config")
    parser.add_argument("--speckle-diff", type=float, required=False,
                        help="Override speckle diff from config")
    parser.add_argument("--speckle-ratio", type=float, required=False,
